...
This document is the canonical architecture and engineering design reference for Codebase Architect v0.1.0 alpha.

The implementation now exists. Therefore:

- statements marked `IMPLEMENTED` describe current repository behavior;
- statements marked `PARTIAL` describe shipped behavior with known limitations;
- statements marked `TARGET` describe future direction, not current capability;
- statements marked `TBD` or `N/A` remain unknown or deliberately out of scope;
- no performance, security, reliability, or compatibility claim should be interpreted as measured evidence unless explicitly marked `[EVIDENCE]`.
...
| `[TARGET]` | Future architecture direction or capability target |
| `IMPLEMENTED` | Present in the v0.1.0 alpha implementation |
...
| Project | Codebase Architect Open-Source Agent Skill & CLI |
| System | Codebase Architect |
| Repository | `https://github.com/YellankiKaushik/Codebase-Architect` |
| Document version | `0.4.0` |
| System version | `0.1.0-alpha` |
| Status | Alpha / Experimental |
...
This document defines the implemented and target architecture, boundaries, data model, processing pipeline, local-AI integration, security model, documentation pipeline, diagram pipeline, reliability strategy, test strategy, release model, and open engineering decisions for Codebase Architect.
...
|---|---|---|---|
| Author | Initial maintainer | Alpha | 2026-09-07 |
...
| 0.4.0 | 2026-09-08 | Initial maintainer | Reconciled with implemented v0.1.0 alpha repository and consolidated canonical architecture document |
...
The first supported execution modes are:
...
**Current implemented state:** Codebase Architect v0.1.0 alpha is a working local CLI and Agent Skill package. It includes deterministic scanning, Python AST analysis, Tree-sitter JavaScript/TypeScript AST analysis when installed with the `javascript` extra, structural JS/TS fallback, framework detection, a Code Intelligence Graph, architecture inference into AIR, cross-file resolver, deterministic diagram/document generation, output validation, caching, benchmark/eval commands, and local model provider adapters.

**Target state:** Deeper semantic/type-aware resolution, larger fixture coverage, richer renderer integrations, and optional model-backed evaluations remain future work. Current alpha documentation must distinguish implemented behavior from targets.
...
| Dimension | Current v0.1.0 Alpha State |
|---|---|
| Architecture style | Modular CLI application with plugin/adaptor architecture |
| Primary implementation language | Python 3.11+ |
| Secondary parsing/runtime integrations | Python AST, optional Tree-sitter JavaScript/TypeScript AST, structural JS/TS fallback |
| Frontend | N/A for MVP; optional VS Code extension later |
| Backend/server | N/A for MVP; local process |
| Metadata store | JSON artifacts and local analysis cache |
| Graph representation | In-memory typed graph persisted as normalized JSON |
| Cache | Local content-addressed cache |
| Messaging | N/A for MVP |
| Cloud/platform | None required |
| Deployment | Local Python package / GitHub Agent Skill |
| Authentication | None for local process |
| Authorization | OS filesystem permissions |
| AI provider | Provider adapter; no-LLM, Ollama, and OpenAI-compatible local modes |
| Local model target | User-selected local instruction/coding model |
| CI/CD | GitHub Actions matrix for Python 3.11, 3.12, and 3.13 plus packaging |
| Observability | Run manifest, validation report, CLI JSON output, benchmark metrics |
| Diagram formats | Mermaid sources and static SVG overview; external renderers remain optional |

## 1.4 Implementation Status Matrix

| Capability | Status | Notes |
|---|---|---|
| Python AST analyzer | IMPLEMENTED | Uses Python `ast` for imports, symbols, decorators, calls, routes, and environment references. |
| Tree-sitter JS/TS AST backend | IMPLEMENTED | Available when installed with the `javascript` extra. |
| Structural JS/TS fallback | IMPLEMENTED | Used when Tree-sitter dependencies or parsing are unavailable. |
| Cross-file resolver | IMPLEMENTED | Resolves local imports, tsconfig aliases, and selected symbol/call relationships; dynamic runtime behavior remains partial. |
| Framework detection | IMPLEMENTED | Detects common Python and JS/TS framework signals from graph and manifest evidence. |
| CIG / AIR | IMPLEMENTED | Typed graph and architecture representation are emitted as JSON evidence artifacts. |
| Benchmark command | IMPLEMENTED | `codebase-architect benchmark . --json` reports duration, cache hit rate, phase timings, and graph stats. |
| Ollama provider | IMPLEMENTED | Local adapter exists; real runtime verification is optional and environment-dependent. |
| OpenAI-compatible provider | IMPLEMENTED | Adapter exists for compatible local endpoints; real runtime verification is optional and environment-dependent. |
| Deep TypeScript type-checker semantics | TARGET | Tree-sitter AST is implemented; full compiler/type-aware semantics are not claimed. |
| Production maturity | PARTIAL | v0.1.0 is alpha/experimental, not production-certified. |
...
|---|---|---|---|---|
| ASM-001 | Repository source is locally readable | Analysis cannot run | Core | IMPLEMENTED assumption |
| ASM-002 | Most architecture-relevant structure is statically discoverable or inferable | Documentation quality falls | Analyzer | PARTIAL |
| ASM-003 | Git is commonly available | Incremental change detection needs fallback hashing | CLI | IMPLEMENTED fallback |
| ASM-004 | Local users can run Ollama or another compatible inference server when AI synthesis is desired | LLM features unavailable; deterministic reports still possible | LLM | OPTIONAL |
| ASM-005 | Users accept confidence labels for dynamic/incomplete analysis | Otherwise tool may create false certainty | Product | IMPLEMENTED |
...
Measured response: PARTIAL — deterministic no-LLM mode and provider health checks are implemented; real local runtime verification is optional.
```
...
Measured response: PARTIAL — streaming discovery, exclusions, cache reuse, and benchmark metrics are implemented; very large monorepo limits remain target work.
```
...
Measured response: IMPLEMENTED for synthetic secret fixtures and output validation; broader DLP coverage remains partial.
```
...
|---|---|---|---|
| Core language | Python | 3.11+ | Orchestration and analyzer/provider APIs |
| CLI | argparse | Python bundled | CLI UX |
| Data model | dataclasses | Python bundled | Typed schemas |
| Graph | Custom typed graph | IMPLEMENTED | Traversal/analysis |
| Metadata store | JSON files + local cache | IMPLEMENTED | Persistence |
| Generic JS/TS syntax | Tree-sitter | optional extra | JavaScript/TypeScript AST extraction |
| TypeScript semantic analysis | TypeScript compiler API / tsserver helper | TARGET | Deeper type-aware imports/symbols/types |
| Python analysis | `ast` | Python bundled | Symbols/imports/calls |
| Ignore engine | pathlib/fnmatch-style exclusions | IMPLEMENTED | Repository filtering |
| Local AI | Ollama HTTP API | IMPLEMENTED adapter | Local inference |
| Local model | User-selected | recorded per run | Semantic synthesis |
...
| Visual renderer | SVG | IMPLEMENTED basic overview | Nontechnical visual view |
| Testing | unittest | IMPLEMENTED | Core tests |
| CI | GitHub Actions | IMPLEMENTED | Build/test/release gate |

Unpinned external tool versions remain `TBD` until a lock or release artifact policy is adopted.
...
codebase-architect inspect [PATH]
codebase-architect doctor
codebase-architect providers
codebase-architect models
codebase-architect eval
codebase-architect benchmark [PATH]
codebase-architect skill install [PATH]
```
...
| `--provider PROVIDER` | No | Select provider |
...
The current v0.1.0 alpha persists JSON artifacts and a local cache rather than a SQLite database. The following tables remain a `[TARGET]` model if a SQLite-backed store is introduced.
...
Latency targets remain TBD until broader repository benchmark evidence is collected.
...
`PARTIAL — a benchmark command exists and reports run duration, files-per-second, cache hit rate, phase durations, and graph statistics. This document does not publish fixed benchmark numbers because they depend on repository size, cache state, hardware, and selected model/provider.`
...
`PARTIAL — cache rebuild behavior is covered by local analysis behavior and tests, but formal disaster-recovery drills are not yet published.`
...
The current project uses `python -m pip install -e ".[javascript]"` for full JS/TS AST development and CI-equivalent testing.
...
```bash
codebase-architect analyze tests/fixtures/typescript_web_api \
  --no-llm \
  --output .tmp/docs
```
...
Optional local provider environment files are user-managed:
...
Exact bootstrap command is `codebase-architect doctor`.
...
```bash
codebase-architect analyze tests/fixtures/typescript_web_api --no-llm
```
...
**Status:** Accepted / IMPLEMENTED for v0.1.0 alpha.
...
Initial technical debt for v0.1.0 alpha is `PARTIAL` and tracked as known limitations rather than accepted architecture exceptions.
...
Current v0.1.0 alpha limitations:
...
No accepted architecture exceptions currently exist for v0.1.0 alpha.
...
No measured production-scale capacity is published.

Required benchmark/forecast format for future larger-scale evidence:
...
v0.1.0 alpha is the current implemented state.
...
|---|---|---|---|---|
| OQ-001 | Final public project/repository name? | Initial maintainer | 2026-09-08 | CLOSED — Codebase Architect |
| OQ-002 | Python Typer vs Click? | Core | 2026-09-08 | CLOSED — argparse for v0.1.0 alpha |
| OQ-003 | NetworkX vs custom graph operations only? | Graph | 2026-09-08 | CLOSED — custom graph for v0.1.0 alpha |
...
| ADR | Decision | Status |
|---|---|---|
| ADR-001 | CLI engine rather than prompt-only skill | Accepted / IMPLEMENTED |
| ADR-002 | CIG as core repository representation | Accepted / IMPLEMENTED |
| ADR-003 | Single AIR for all diagrams | Accepted / IMPLEMENTED |
| ADR-004 | Provider-agnostic LLM; Ollama first | Accepted / IMPLEMENTED |
| ADR-005 | Content-addressed incremental analysis | Accepted / IMPLEMENTED |
| ADR-006 | Dual secret-redaction boundary | Accepted / IMPLEMENTED |
| ADR-007 | Python core | Accepted / IMPLEMENTED |
...
`PARTIAL — implemented alpha behavior exists, with remaining limitations documented above.`
