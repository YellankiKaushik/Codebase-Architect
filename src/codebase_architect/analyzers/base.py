...
def evidence(path: str, content_hash: str, line: int | None = None, symbol: str | None = None, note: str | None = None) -> Evidence:
    return Evidence(
        id=f"EVD-{stable_id(path, str(line), symbol or '', note or '')}",
        path=path, line_start=line, line_end=line, symbol=symbol,
        content_hash=content_hash, note=note,
    )

def file_node(path: str, content_hash: str, language: str) -> Node:
    return Node(
        id=f"FILE-{stable_id(path)}", kind="FILE", name=Path(path).name, path=path,
        properties={"language": language},
        evidence=[evidence(path, content_hash, 1, note="file exists")],
    )
