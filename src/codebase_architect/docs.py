...
def write_documentation(repository: Path, output: Path, air: ArchitectureIR, graph: CodeGraph, stats: RunStats, commit: str | None) -> list[Path]:
    output.mkdir(parents=True, exist_ok=True)
    (output/"components").mkdir(exist_ok=True)
    evidence_dir=output/"evidence"; evidence_dir.mkdir(exist_ok=True)
    created=[
        _write_guarded(output/"README.md", _index(air)),
        _write_guarded(output/"DEEP_TECHNICAL_ARCHITECTURE.md", _master(repository,air,graph,stats,commit)),
    ]
    for component in air.components:
        created.append(_write_guarded(output/"components"/f"{_slug(component.name)}.md", _component_doc(component)))
    created.append(_write_guarded(evidence_dir/"evidence-index.md", _evidence_index(graph)))
...
def _write_guarded(path:Path,content:str)->Path:
    if path.exists():
